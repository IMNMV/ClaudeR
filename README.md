...
- **Windows multi-session safety warning.** Never use a build whose stale discovery cleanup probes liveness with `tools::pskill(pid, signal = 0)` on Windows. That path has terminated live RStudio sessions on this machine when another session starts. Use a read-only PID probe and run the local multi-session regression before trusting a modified build.
